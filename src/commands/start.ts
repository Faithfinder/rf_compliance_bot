...
export const START_INTRO = [
    ...
    "",
    "⚠️ Это размен удобства на надёжность. Модерация удобнее — публикуете как обычно, — но проверяет уже опубликованный пост: подписчики успевают получить уведомление, а если бот недоступен, немаркированный пост останется в канале, и никто об этом не узнает. Публикация через бота неудобнее, зато проверяет до выхода поста: если бот недоступен, пост просто не выйдет.",
    "",
...
export function buildStartStatus(channelConfig: ChannelConfig, requirements: ChannelRequirements): FormattedString {
    ...
    if (moderationRequirementsPassed(requirements)) {
        ...
        if (!publishRequirementsPassed(requirements)) {
            sections.push(
                "Модерация удаляет пост уже после публикации: подписчики успевают его увидеть, а если я буду недоступен — он и вовсе останется в канале. Публикация через бота менее удобна, но проверяет до выхода поста. Как её включить — /info.",
            );
        }
    } else {
