...
| `POSTHOG_ID_SALT` | Соль для HMAC, которым хешируются Telegram ID пользователей. Если не задана, используется `TELEGRAM_BOT_TOKEN` — и смена токена обнулит статистику по пользователям | нет |
...
Telegram ID пользователя уходит исключительно как HMAC-SHA256 с солью из `POSTHOG_ID_SALT`, усечённый до 128 бит. Соль не покидает процесс, поэтому восстановить ID из хеша нельзя. Если соль не задана, ею служит `TELEGRAM_BOT_TOKEN` — тоже секрет, но со следствием: после смены токена все пользователи в статистике станут новыми. Если соли нет вообще (не задан ни один из двух параметров), телеметрия не включится: отправлять хеши, которые можно подобрать перебором, хуже, чем не собирать ничего.
