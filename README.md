...
Бот умеет отправлять продуктовую аналитику в [PostHog](https://posthog.com/) — чтобы владелец видел, доходят ли администраторы до конца настройки и как часто ловятся посты без маркировки. **По умолчанию телеметрия выключена:** без `POSTHOG_API_KEY` все вызовы аналитики превращаются в пустышку и ни одного сетевого запроса не уходит.

Аудитория бота — администраторы каналов, попавших под закон об «иностранных агентах», поэтому данные о людях и данные о каналах обрабатываются по-разному.
...
**Пользователи: только хеш.** Telegram ID уходит исключительно как HMAC-SHA256 с солью из `POSTHOG_ID_SALT`, усечённый до 128 бит; соль не покидает процесс. У соли нет значения по умолчанию: если задан `POSTHOG_API_KEY`, а соль — нет, бот не запустится. Это намеренно — хеш с предсказуемой солью подбирается перебором, ID пользователей Telegram легко перечислить целиком. Сгенерируйте соль один раз (`openssl rand -hex 32`) и сохраните: при её смене все пользователи в статистике станут новыми. Получателей уведомлений это касается тоже — в аналитику попадает только их количество.

Если автора определить нельзя — а для постов в канале Telegram обычно не сообщает, кто их опубликовал — событие уходит **вообще без идентификатора пользователя**, и PostHog не создаёт профиль. Подставного идентификатора вместо настоящего не придумывается.

**Каналы: в открытом виде.** ID и названия каналов отправляются как есть: канал, который модерирует бот, — публичная информация, а читаемые названия — единственное, что делает аналитику по каналам осмысленной. Считаются не только нарушения, но и пропущенные посты: без них доля соблюдения не вычисляется — канал, где все ставят маркировку правильно, выглядел бы так же, как канал, где бот сломался.

**Единственное исключение про текст пользователя.** Если сообщение начинается с `/`, записывается имя команды — в том числе нераспознанной, чтобы находить опечатки. **Аргументы не записываются никогда**: `/notify_add 123456789` уходит как `notify_add` плюс признак «аргументы были». Имя нераспознанной команды, похожее на идентификатор (`/123456789`), заменяется на `redacted`. Из обычных сообщений не записывается ничего.
