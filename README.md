...
Дело не в удобстве, а в том, **как каждый режим ломается**:
...
| Удобство | публикуете как обычно | пишете боту в личные сообщения, форматирование ограничено возможностями `copyMessage` |

То есть модерация отказывает «в открытую» и молча, а публикация через бота — «в закрытую» и заметно. Именно поэтому второй режим неудобнее, но строже: молчаливый пропуск немаркированного поста — ровно то, ради чего бот и существует.
