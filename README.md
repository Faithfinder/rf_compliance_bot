...
Считаются не только нарушения, но и посты, которые бот проверил и **пропустил** — без них не с чем сравнивать: канал, где все ставят маркировку правильно, выглядел бы точно так же, как канал, где бот сломался или где ничего не публикуют. Доля соблюдения — это отношение пропущенных постов к сумме пропущенных и удалённых.

Отдельно отмечаются каналы, в которых бот сидит, но текст маркировки так и не настроили: событие отправляется **один раз на канал за время работы процесса**, а не на каждый пост. После перезапуска счёт начинается заново, поэтому количество таких событий — это не количество каналов; смотреть нужно на число уникальных каналов в разбивке.

Новых категорий данных это не добавляет: те же ID и название канала, что и раньше.
...
Если автора определить нельзя — а для постов в канале Telegram обычно не сообщает, кто их опубликовал — событие уходит **вообще без идентификатора пользователя**, и PostHog не создаёт для него профиль. Подставного идентификатора вместо настоящего не придумывается. Разбор по каналам при этом не теряется: он делается по ID и названию канала, которые и так отправляются.
